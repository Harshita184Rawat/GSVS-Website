...
?>
    <table class="table wpf_order_items_table wpf_table table_bordered">
        <thead>
        <th><?php _e('Item', 'wp-payment-form'); ?></th>
        <th><?php _e('Quantity', 'wp-payment-form'); ?></th>
        <th><?php _e('Price', 'wp-payment-form'); ?></th>
        <th><?php _e('Line Total', 'wp-payment-form'); ?></th>
        </thead>
        <tbody>
        <?php $subTotal = 0; ?>
        ...
        if (isset($submission->discounts['applied']) && count($submission->discounts['applied'])) : ?>
            <tr class="wpf_total_row">
                <th style="text-align: right" colspan="3"><?php _e('Sub-Total', 'wp-payment-form'); ?></th>
            ...
            foreach ($submission->discounts['applied'] as $discount) :
                ...
                <tr class="wpf_discount_row">
                    <th style="text-align: right"
                        colspan="3"><?php _e('Discounts (' . $discount->item_name . ' )', 'wp-payment-form'); ?></th>
        ...
        <?php if ($submission->tax_items->count()) : ?>
            <tr class="wpf_sub_total_row">
                <th style="text-align: right" colspan="3"><?php _e('Sub Total', 'wp-payment-form'); ?></th>
                <td><?php echo wpPayFormFormattedMoney($subTotal - $discountTotal, $currencySetting); ?></td>
            </tr>
            <?php foreach ($submission->tax_items as $tax_item) : ?>
                <tr class="wpf_sub_total_row">
                    <td style="text-align: right" colspan="3"><?php echo esc_html($tax_item->item_name); ?></td>
                    <td><?php echo wpPayFormFormattedMoney($tax_item->line_total, $currencySetting); ?></td>
                </tr>
            <?php endforeach; ?>
        <?php endif; ?>
        <tr class="wpf_total_payment_row">
            <th style="text-align: right" colspan="3"><?php _e('Total', 'wp-payment-form'); ?></th>
 ...
 if ($submission->payment_method === 'paypal' && $submission->payment_status == 'pending') {?>
    <div style="background: #f7fafc; border: 1px solid #cac8c8; padding: 10px; font-size:13px; margin-bottom: 12px;">
        <h3>Payment is not marked as paid yet.</h3>
        Sometimes, PayPal payments take a few moments to mark as paid!
        Try reloading receipt page after sometime.
    </div>
<?php }?>
