        ...
        if ($elements) {
            ...
            if (!empty($isStepForm)) {
                ...
                foreach ($isStepForm['editor_elements']['form_steps'] as $key=>$elements) {
                    ...
                    if (sizeof($isStepForm['editor_elements']['form_steps']) == $key + 1) {
                        ?>
                    <button class="wpf_step_button" id="wpf_step_prev">&larr; <?php echo __('Previous') ?></button>
                    <button class="wpf_step_button" id="wpf_step_next"><?php echo __('Next') ?> &rarr;</button>
        ...
        $buttonAttributes = apply_filters('wppayform/submit_button_attributes', array(
            ...
            <?php if ($form->turnstile_status) : ?>
                <div class="wpf_form_group wpf_form_turnstile">
                    <div data-sitekey="<?php echo esc_attr($form->turnstile_site_key); ?>" 
                        id="wpf-turnstile_<?php echo intval($form->ID); ?>" 
                        class='wpf-el-turnstile cf-turnstile' data-callback='turnstileCallback'>
                    </div>
                </div>
            <?php endif ?>
    ...
    private function addAssets($form, $instanceCssClass)
    {
        ...
        wp_localize_script('wppayform_public', 'wp_payform_general', array(
            ...
            'i18n' => array(
                ...
                'button_state' => __($processingText, 'wp-payment-form'),
